...
- **`/multiagente-init`** — inicia um projeto novo: Discovery (perguntas),
  Fase 1 (docs numerados), Fase 2 (time de agentes), Fase 3 (setup +
  git + repo GitHub), spawn dos especialistas, abre dashboard.
- **`/multiagente-spawn <agente>`** — spawna um especialista novo num
  projeto já iniciado.
- **`/multiagente-dashboard`** — abre (ou reabre) o dashboard tmux no
  Tilix.
...
- **Skill de bootstrap** (`skills/multiagente-workflow`): Claude lê e
...
2. `/multiagente-init` — o Arquiteto faz Discovery (stack, objetivo,
